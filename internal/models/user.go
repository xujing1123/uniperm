...
type (
	...
	UnipermUser struct {
		...
		Username    string `gorm:"column:username;type:varchar(20);not null;default:'';comment:用户名;unique" json:"username"`          // 用户名
		Password    string `gorm:"column:password;type:varchar(32);not null;default:'';comment:密码（MD5加密）" json:"password"`           // 密码（MD5加密）
		...
		State       byte   `gorm:"column:state;type:tinyint;not null;default:1;comment:状态：1启用 2禁用" json:"state"`                     // 状态：1启用 2禁用

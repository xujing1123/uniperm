...
type (
	...
	IdReq struct {
		Id uint `validate:"min(1,用户Id不能为空)" json:"id" log:"用户Id"`
	}
	GetReq           IdReq
	...
	LoginReq struct {
		Username string `validate:"minlength(1,用户名不能为空) maxlength(20,用户名长度不能超过20)" json:"username" log:"用户名"`
		Password string `validate:"minlength(1,密码不能为空) maxlength(20,密码长度不能超过20)" json:"password" log:"-"`
		ClientIP string `validate:"maxlength(20,客户端IP长度不能超过20)" json:"client_ip" log:"客户端IP"`
		unilog.UC

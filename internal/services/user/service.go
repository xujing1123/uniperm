...
func (s *service) Login(req LoginReq) (resp LoginResp, err error) {
	...
	if user.Id == 0 {
		...
		_ = base.Callback1(nil, req, req.Callback.NotFound)
		return
	}
	if user.State == models.UserStateDisable {
		err = errors.New("用户被禁用，请联系管理员")
		_ = base.Callback2(nil, req, user, req.Callback.Disabled)
		return
	}
	if user.RoleId > 0 {
		var roleState byte
		...
		if roleState == models.RoleStateDisable {
			...
			_ = base.Callback2(nil, req, user, req.Callback.RoleDisabled)
			return
		}
	}
	if pwd := pkg.MD5(req.Password); pwd != user.Password {
		err = errors.New("用户名和密码不匹配，请确认后重试")
		_ = base.Callback2(nil, req, user, req.Callback.PasswordWrong)
		return
	}
